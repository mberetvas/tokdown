...
todos:
  ...
  - id: ruff-enforcement
    ...
    content: "pyproject ruff: TID252 + banned-api for *internal; per-file-ignores for [api.py](http://api.py) and tests/domain/*internal"
  ...
  - id: tdd-code-blocks
    ...
    content: "TDD slice: code-fence state machine — tests/domain/_internal/test_chunking_[service.py](http://service.py)"
  ...
  - id: application-ports
    ...
    content: application/[ports.py](http://ports.py) DocumentGateway + PartFileExistsError; re-export via application/[api.py](http://api.py)

    status: pending

  - id: domain-facade

    content: domain/[api.py](http://api.py) pure split surface + domain/_internal/* (no I/O ports)

    status: pending

  - id: application-facade

    content: application/[api.py](http://api.py) + _internal use case (driven by application tests)

    status: pending

  - id: infrastructure-facade

    content: infrastructure/[api.py](http://api.py) + _internal adapters (driven by infra tests)

    status: pending

  - id: interface-facade

    content: interface/[api.py](http://api.py) + _internal cli/composition (driven by interface tests)

    status: pending

  - id: logging-domain

    content: domain/logging/[api.py](http://api.py); infrastructure logging _internal
...
"tokdown.domain._internal" = "Import tokdown.domain.api only (except domain/[api.py](http://api.py) and tests/domain/_internal/)."

[tool.ruff.lint.per-file-ignores]

"src/tokdown/domain/[api.py](http://api.py)" = ["TID251"]

"src/tokdown/application/[api.py](http://api.py)" = ["TID251"]

"src/tokdown/infrastructure/[api.py](http://api.py)" = ["TID251"]

"src/tokdown/interface/[api.py](http://api.py)" = ["TID251"]
...
| `StructuredLogger`                                                       | **domain/logging/[api.py](http://api.py)** | Cross-cutting port (no filesystem)                                                                                   |
...
- `test_markdown_regions.py`: tildes `~~~`; trailing spaces; info string ````bash [script.s](http://script.sh)`; 4-backtick markers if matched by` {3,}`.
...
tests/
  ...
  [conftest.py](http://conftest.py)              # shared fixtures: tmp paths, sample markdown, fake logger

  domain/

    test_splitting_[api.py](http://api.py)  # Facade: major behaviors (limits, multi-part, code block preserved)

    _internal/

      test_chunking_[service.py](http://service.py)   # Direct: fence toggling, prose splits, malformed fence

      test_markdown_[regions.py](http://regions.py)   # Direct: region iterator edge cases

  domain/logging/

    test_logging_[api.py](http://api.py)    # LogEvent constants; fake logger contract

  application/

    test_application_[api.py](http://api.py)

  infrastructure/

    test_token_[encoders.py](http://encoders.py) # TiktokenEncoder, HuggingFaceEncoder (mark slow/network)

    test_filesystem_[gateway.py](http://gateway.py)

    test_json_[logger.py](http://logger.py)

  interface/

    test_main_[cli.py](http://cli.py)       # interface.api.main end-to-end with tmp_path

  fakes/

    fake_document_[gateway.py](http://gateway.py)

    fake_[logger.py](http://logger.py)

    fake_token_[encoder.py](http://encoder.py)  # deterministic encode/decode for domain tests
...
src/tokdown/
  ...
  interface/
    ...
    [api.py](http://api.py)                       # main()

    _internal/

      [cli.py](http://cli.py)

      [composition.py](http://composition.py)

  application/

    [api.py](http://api.py)                       # SplitDocumentApplication, Request/Result DTOs

    [ports.py](http://ports.py)                     # DocumentGateway ABC, PartFileExistsError

    _internal/

      split_[document.py](http://document.py)          # use case: load → split → save

  domain/

    [api.py](http://api.py)                       # DocumentSplittingDomain + ChunkLimit/ChunkSizer only

    _internal/

      value_[objects.py](http://objects.py)           # ChunkUnit, ChunkLimit only

      markdown_[regions.py](http://regions.py)        # FENCE_LINE regex + iter_regions()

      [sizing.py](http://sizing.py)                  # ChunkSizer, TokenEncoder protocol, Token/WordChunkSizer

      [services.py](http://services.py)                # MarkdownChunkingService + auto-healing

    logging/

      [api.py](http://api.py)                     # StructuredLogger, LogLevel, LogEvent

  infrastructure/

    [api.py](http://api.py)                       # create_infrastructure, Infrastructure, InfraSettings

    _internal/

      filesystem_[gateway.py](http://gateway.py)

      tiktoken_[encoder.py](http://encoder.py)

      huggingface_[encoder.py](http://encoder.py)

      token_encoder_[factory.py](http://factory.py)

      sizing_[factory.py](http://factory.py)

      logging/

        json_[logger.py](http://logger.py)

        [sanitizer.py](http://sanitizer.py)
...
uv run tokdown --provider openai [test.md](http://test.md) 100

uv run tokdown --provider google [test.md](http://test.md) 100

uv run tokdown --words [test.md](http://test.md) 50
